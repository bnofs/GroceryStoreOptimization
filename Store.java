...
public class Store {
  ...
  private Node start; //The start square, where the maze begins.
  private Node goal; //The goal square, where the maze ends.
  private Set<Node> obstacles; //Squares within the maze that cannot be traveled through.
    ...
    try {
      ...
      while (in.hasNextLine()) {
        ...
        for (int j = 0; j < line.length(); j++) {
          if (line.charAt(j) == 'S') {
            this.start = new Node(this.rows, j);
          } else if (line.charAt(j) == 'G') {
            this.goal = new Node(this.rows, j);
          } else if (line.charAt(j) == '#') {
            this.obstacles.add(new Node(this.rows, j));
          }
        }
        this.rows++;
      }
   ...
   */
  public boolean isBlocked(Node square) {
  ...
  public String toString() {
    ...
    for (int row = 0; row < this.rows; ++row) {
      ...
      for (int column = 0; column < this.columns; ++column) {
        Node square = new Node(row, column);
